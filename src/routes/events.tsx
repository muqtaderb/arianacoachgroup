...
function EventsPage() {
  const { data: all = [] } = useQuery({ queryKey: ["events"], queryFn: () => fetchEvents() });
  const featured = all.find((e) => e.is_featured);
  const upcoming = all.filter((e) => !e.is_featured);
  return (
    <PageShell
      eyebrow="Events"
    ...
    >
      ...
      <div className="overflow-hidden rounded-3xl border border-border bg-card shadow-elegant">
        <div className="grid gap-0 md:grid-cols-2">
          <div className="bg-hero-gradient p-10 text-white md:p-14">
            ...
            <h2 className="mt-3 text-3xl font-bold tracking-tight md:text-4xl">{featured.title}</h2>
            <p className="mt-4 text-white/75">{featured.description}</p>
            <div className="mt-8 space-y-3 text-sm">
              <div className="flex items-center gap-3"><Calendar className="h-4 w-4 text-brand-orange" /> {featured.event_date}</div>
              <div className="flex items-center gap-3"><MapPin className="h-4 w-4 text-brand-orange" /> {featured.location}</div>
      ...
      <div className="mt-20">
        ...
        <div className="mt-8 grid gap-6 md:grid-cols-3">
          {upcoming.map((e) => (
            <div key={e.id} className="rounded-2xl border border-border bg-card p-6 shadow-soft">
              ...
              <div className="mt-5 space-y-1.5 text-xs text-muted-foreground">
                <div className="flex items-center gap-2"><Calendar className="h-3.5 w-3.5" /> {e.event_date}</div>
